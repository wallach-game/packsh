...
A packer for bash tools. Point it at a local repo with a `packsh.json` and it
...
installed bashumerate
  bin: enumerate -> /home/jirka/.packsh/bin/enumerate
```

## Usage

```bash
packsh -l <path/to/repo>              # build, then install
packsh -test <path/to/repo>           # build, then run the package's tests against it
packsh --single-bundle <path/to/repo> # build, and emit one runnable .sh
packsh -u <name>                      # uninstall
packsh --list                         # list installed packages
packsh -v | -h                        # version / help
```

Flags can appear in any order, and `--single-bundle` combines with the others:
`packsh -l --single-bundle .` and `packsh --single-bundle -l .` are the same
thing.

Remote installs (`packsh user/repo`) are **not supported** — local only.
...
They are **not** declared in `packsh.json`, so the same list never lives in two
places. If the repo has a `package.sh` (basher's manifest format), packsh reads
it:
...
The dist is **copied** into `$PACKSH_HOME/cellar/<name>` (default
`~/.packsh`), and bins and completions are symlinked out into
`$PACKSH_HOME/bin` and `$PACKSH_HOME/completions/`. Add `~/.packsh/bin` to
your `PATH`.
...
root with `PACKSH_DIST` pointing at the build. A suite that honours it tests
...
Run bare (`bats test`) it still tests the source; run through packsh it tests
...
cache at `$PACKSH_HOME/vendor` (once, then reused) and shipped inside the
...
To build it, packsh strips what inlining makes redundant: shebangs, `*_ROOT`
...
packsh requires [`jq`](https://jqlang.org/) to parse `packsh.json`, and
`git` to fetch `DEPS`. Both are system prerequisites, like basher requiring
`git` — packsh does not manage them.
